...
//get users and set users
handle[353] = function (line,connection) {
    var users = line.split(":");
    var usrArr = users[2].split(" ");
    connection.users = usrArr;
    var ctx = {};
    ctx.type = "names";
    ctx.from = "";
    ctx.to = "";
...
handle["PRIVMSG"] = function (line,connection) {
    ...
    ctx.pm = false;
    ctx.from = user[0];
    ctx.to = line[1].split(" ")[2];
    ctx.message = escape(line.splice(2,line.length).join(":"));

    var me = connection.username;
    if(ctx.to === me) {
        ctx.pm = true;
    }

    ctx.message = linkify(ctx.message);
    ctx.message = emotes.emojify(ctx.message);
